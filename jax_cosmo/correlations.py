...
def windowfn(x, dlnxleft=0.46, dlnxright=0.46):
    ...
    w = jax.ops.index_update(w, indices, 1.)
    ...
    w = jax.ops.index_update(w, il_indices, rl - jnp.sin(jnp.pi * 2 * rl) / (2 * jnp.pi))
    w = jax.ops.index_update(w, ir_indices, rr - jnp.sin(jnp.pi * 2 * rr) / (2 * jnp.pi))

    return w

def calc_Mellnu(tt, alpha, q=0):
    n = q - 1 - 1j * tt
    intjlttn = 2**(n-1) * jnp.sqrt(jnp.pi) * \
